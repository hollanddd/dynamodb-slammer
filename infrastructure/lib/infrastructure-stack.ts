...
import { Dashboard, GraphWidget, Metric, Unit } from 'aws-cdk-lib/aws-cloudwatch';
...
export class DynamoWritesStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const batchPutFunction = new NodejsFunction(this, 'DynamodbBatchFunction', {
      ...
      entry: join(__dirname, '..', '..', 'handlers', 'dynamodb-batch', 'index.ts'),
      environment: {
    ...
    const queueProducerFunction = new NodejsFunction(this, 'QueueProducerFunc', {
      timeout: cdk.Duration.seconds(12),
      memorySize: 512,
      bundling: {
        commandHooks: {
      ...
      entry: join(__dirname, '..', '..', 'handlers', 'queue-producer', 'index.ts'),
      environment: {
    ...
    const queueConsumerFunction = new NodejsFunction(this, 'QueueConsumerFunc', {
      timeout: cdk.Duration.seconds(12),
      memorySize: 512,
      entry: join(__dirname, '..', '..', 'handlers', 'queue-consumer', 'index.ts'),
      environment: {
        TABLE_NAME: queueProducerTable.tableName,
      },
    });

    queueConsumerFunction.addEventSource(new SqsEventSource(queue, {
      batchSize: 5,
